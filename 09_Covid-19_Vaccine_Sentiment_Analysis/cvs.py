...
w1_dict = dict()
# Count word frequencies in most positive tweets, skipping stopwords
...
# Sort positive word frequencies in descending order
w1_dict = {k: v for k, v in sorted(w1_dict.items(), key=lambda item: item[1], reverse=True)}

l_t = Most_Negative_text
w2_dict = dict()
# Count word frequencies in most negative tweets, skipping stopwords
...
# Sort negative word frequencies in descending order
w2_dict = {k: v for k, v in sorted(w2_dict.items(), key=lambda item: item[1], reverse=True)}

# Select top 10 frequent words from positive and negative tweets
top_10_pos = list(w1_dict.keys())[:10]
top_10_neg = list(w2_dict.keys())[:10]

# Visualize top 10 positive words as a word cloud
plt.subplot(1, 2, 1)
...
# Visualize top 10 negative words as a word cloud
plt.subplot(1, 2, 2)
...
print("\nDisplaying word clouds for top 10 positive and negative words...")
plt.show()